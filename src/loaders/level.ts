...
import { LevelTimer } from '../traits/data/LevelTimer'
import { Trigger } from '../traits/data/Trigger'
...
function loadPattern(name: string) {
  return loadJSON<LevelSpecPatterns>(urlResolver(`sprites/patterns/${name}.json`))
}
...
export function createLevelLoader(entityFactory: EntityFactoryDict) {
  return async function loadLevel(name: string) {
    const levelSpec = await loadJSON<LevelSpec>(urlResolver(`levels/${name}.json`))
