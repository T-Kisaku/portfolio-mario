...
async function main(canvas: HTMLCanvasElement) {
  ...
  const takashi = entityFactory.takashi?.() || raise('where takashi tho')
  makePlayer(takashi, 'TAKASHI')

  const inputRouter = setupKeyboard(window)
  inputRouter.addReceiver(takashi)
  ...
  async function runLevel(name: string) {
    ...
    takashi.pos.set(0, 1)
    takashi.vel.set(0, 0)
    level.entities.add(takashi)

    const playerEnv = createPlayerEnv(takashi)
